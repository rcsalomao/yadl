...
struct Deterministic : RV {
  ...
  double sample() override { return m_value; }
...
struct Normal : RV {
  ...
  Normal(const gsl_rng_type *rngType, double mu = 0.0, double sigma = 1.0,
  ...
  ~Normal() { gsl_rng_free(m_rng); }
  void set_rng(gsl_rng *newRNG) override {
    gsl_rng_free(m_rng);
    m_rng = newRNG;
  }
  void set_seed(unsigned long int seed) override { gsl_rng_set(m_rng, seed); }
  void set_values(double mu, double sigma) {
    m_mu = mu;
    m_sigma = sigma;
  }
  double sample() override { return m_mu + gsl_ran_gaussian(m_rng, m_sigma); }
...
struct Lognormal : RV {
  ...
  Lognormal(const gsl_rng_type *rngType, double zeta = 0.0, double sigma = 1.0,
            unsigned long int seed = std::random_device()())
  ...
  double sample() override { return gsl_ran_lognormal(m_rng, m_zeta, m_sigma); }
...
struct Gumbel : RV {
  ...
  Gumbel(const gsl_rng_type *rngType, double a = 0.0, double b = 1.0,
         unsigned long int seed = std::random_device()())
      : m_a(a), m_b(b) {
    m_rng = gsl_rng_alloc(rngType);
    gsl_rng_set(m_rng, seed);
  }
  Gumbel(double a = 0.0, double b = 1.0,
         unsigned long int seed = std::random_device()())
      : m_a(a), m_b(b) {
    m_rng = gsl_rng_alloc(gsl_rng_ranlxd2);
    gsl_rng_set(m_rng, seed);
  }
  ~Gumbel() { gsl_rng_free(m_rng); }
  void set_rng(gsl_rng *newRNG) override {
    gsl_rng_free(m_rng);
    m_rng = newRNG;
  }
  ...
  double sample() override { return gsl_ran_gumbel1(m_rng, m_a, m_b); }
...
struct Weibull : RV {
  ...
  Weibull(const gsl_rng_type *rngType, double a = 1.0, double b = 1.0,
          unsigned long int seed = std::random_device()())
      : m_a(a), m_b(b) {
    m_rng = gsl_rng_alloc(rngType);
    gsl_rng_set(m_rng, seed);
  }
  Weibull(double a = 1.0, double b = 1.0,
          unsigned long int seed = std::random_device()())
      : m_a(a), m_b(b) {
    m_rng = gsl_rng_alloc(gsl_rng_ranlxd2);
    gsl_rng_set(m_rng, seed);
  }
  ~Weibull() { gsl_rng_free(m_rng); }
  void set_rng(gsl_rng *newRNG) override {
    gsl_rng_free(m_rng);
    m_rng = newRNG;
  }
  ...
  double sample() override { return gsl_ran_weibull(m_rng, m_a, m_b); }
...
struct LevySaS : RV {
  ...
  LevySaS(const gsl_rng_type *rngType, double mu = 0.0, double c = 1.0,
          double alpha = 1.0, unsigned long int seed = std::random_device()())
  ...
  double sample() override { return m_mu + gsl_ran_levy(m_rng, m_c, m_alpha); }
  double mean() override {
    if (m_alpha > 1.0) {
      return m_mu;
    } else {
...
struct LevySkew : RV {
  ...
  LevySkew(const gsl_rng_type *rngType, double mu = 0.0, double c = 1.0,
           double alpha = 0.5, double beta = 1.0,
           unsigned long int seed = std::random_device()())
  ...
  LevySkew(double mu = 0.0, double c = 1.0, double alpha = 0.5,
           double beta = 1.0, unsigned long int seed = std::random_device()())
      : m_mu(mu), m_c(c), m_alpha(alpha), m_beta(beta) {
    assert((m_alpha > 0.0) && (m_alpha <= 2.0));
    assert((m_beta >= -1.0) && (m_beta <= 1.0));
    m_rng = gsl_rng_alloc(gsl_rng_ranlxd2);
...
struct UniformReal : RV {
  ...
  UniformReal(const gsl_rng_type *rngType, double a = 0.0, double b = 1.0,
              unsigned long int seed = std::random_device()())
      : m_a(a), m_b(b) {
    assert(m_a < m_b);
    m_rng = gsl_rng_alloc(rngType);
  ...
  UniformReal(double a = 0.0, double b = 1.0,
              unsigned long int seed = std::random_device()())
      : m_a(a), m_b(b) {
    assert(m_a < m_b);
    m_rng = gsl_rng_alloc(gsl_rng_ranlxd2);
  ...
  ~UniformReal() { gsl_rng_free(m_rng); }
  void set_rng(gsl_rng *newRNG) override {
    gsl_rng_free(m_rng);
    m_rng = newRNG;
  }
  ...
  double sample() override { return gsl_ran_flat(m_rng, m_a, m_b); }
  ...
  double cdf_P_inv(double P) override { return gsl_cdf_flat_Pinv(P, m_a, m_b); }
};
...
struct GeneralDiscrete : RV {
  ...
  GeneralDiscrete(const gsl_rng_type *rngType, std::vector<double> P,
                  unsigned long int seed = std::random_device()())
      : m_P(P) {
  ...
  GeneralDiscrete(std::vector<double> P,
                  unsigned long int seed = std::random_device()())
      : m_P(P) {
    m_table = gsl_ran_discrete_preproc(size(m_P), m_P.data());
    m_rng = gsl_rng_alloc(gsl_rng_ranlxd2);
  ...
  void set_seed(unsigned long int seed) override { gsl_rng_set(m_rng, seed); }
  double sample() override { return gsl_ran_discrete(m_rng, m_table); }
  double mean() override {
    double num{0.0};
    double den{0.0};
...
struct UniformInt : RV {
  ...
  UniformInt(const gsl_rng_type *rngType, int a = 0, int b = 10,
             unsigned long int seed = std::random_device()())
      : m_a(a), m_b(b) {
    assert(m_a < m_b);
    m_rng = gsl_rng_alloc(rngType);
  ...
  void set_seed(unsigned long int seed) override { gsl_rng_set(m_rng, seed); }
  double sample() override { return m_a + gsl_ran_discrete(m_rng, m_table); }
...
struct Exponential : RV {
  ...
  Exponential(const gsl_rng_type *rngType, double mu = 1.0,
              unsigned long int seed = std::random_device()())
      : m_mu(mu) {
    assert(m_mu > 0.0);
    m_rng = gsl_rng_alloc(rngType);
  ...
  Exponential(double mu = 1.0, unsigned long int seed = std::random_device()())
      : m_mu(mu) {
    assert(m_mu > 0.0);
    m_rng = gsl_rng_alloc(gsl_rng_ranlxd2);
    gsl_rng_set(m_rng, seed);
  }
  ~Exponential() { gsl_rng_free(m_rng); }
  void set_rng(gsl_rng *newRNG) override {
    gsl_rng_free(m_rng);
    m_rng = newRNG;
  }
  ...
  double sample() override { return gsl_ran_exponential(m_rng, m_mu); }
  double mean() override { return m_mu; }
...
struct Gamma : RV {
  ...
  Gamma(const gsl_rng_type *rngType, double a = 1.0, double b = 1.0,
        unsigned long int seed = std::random_device()())
      : m_a(a), m_b(b) {
    assert(m_a > 0.0);
    assert(m_b > 0.0);
  ...
  Gamma(double a = 1.0, double b = 1.0,
        unsigned long int seed = std::random_device()())
      : m_a(a), m_b(b) {
    assert(m_a > 0.0);
    assert(m_b > 0.0);
  ...
  double sample() override { return gsl_ran_gamma(m_rng, m_a, m_b); }
...
struct Beta : RV {
  ...
  Beta(const gsl_rng_type *rngType, double a = 1.0, double b = 1.0,
       unsigned long int seed = std::random_device()())
      : m_a(a), m_b(b) {
    assert(m_a > 0.0);
    assert(m_b > 0.0);
  ...
  double sample() override { return gsl_ran_beta(m_rng, m_a, m_b); }
  ...
  double cdf_P_inv(double P) override { return gsl_cdf_beta_Pinv(P, m_a, m_b); }
};
...
struct ScaledBeta : RV {
  ...
  ScaledBeta(const gsl_rng_type *rngType, double a = 1.0, double b = 1.0,
             double infLimit = 0.0, double supLimit = 1.0,
             unsigned long int seed = std::random_device()())
  ...
  double sample() override {
    return m_infLimit +
           (m_supLimit - m_infLimit) * gsl_ran_beta(m_rng, m_a, m_b);
  }
  double mean() override {
...
struct Poisson : RV {
  ...
  Poisson(const gsl_rng_type *rngType, double mu = 1.0,
          unsigned long int seed = std::random_device()())
      : m_mu(mu) {
    assert(m_mu > 0.0);
    m_rng = gsl_rng_alloc(rngType);
  ...
  Poisson(double mu = 1.0, unsigned long int seed = std::random_device()())
      : m_mu(mu) {
    assert(m_mu > 0.0);
    m_rng = gsl_rng_alloc(gsl_rng_ranlxd2);
    gsl_rng_set(m_rng, seed);
  }
  ~Poisson() { gsl_rng_free(m_rng); }
  void set_rng(gsl_rng *newRNG) override {
    gsl_rng_free(m_rng);
    m_rng = newRNG;
  }
  ...
  double sample() override { return gsl_ran_poisson(m_rng, m_mu); }
  double mean() override { return m_mu; }
...
struct Bernoulli : RV {
  ...
  Bernoulli(const gsl_rng_type *rngType, double p = 0.5,
            unsigned long int seed = std::random_device()())
      : m_p(p) {
    assert(m_p >= 0.0);
    assert(m_p <= 1.0);
  ...
  Bernoulli(double p = 0.5, unsigned long int seed = std::random_device()())
      : m_p(p) {
    assert(m_p >= 0.0);
    assert(m_p <= 1.0);
    m_rng = gsl_rng_alloc(gsl_rng_ranlxd2);
  ...
  double sample() override { return gsl_ran_bernoulli(m_rng, m_p); }
...
struct Binomial : RV {
  ...
  Binomial(const gsl_rng_type *rngType, double p = 0.5, int n = 1,
           unsigned long int seed = std::random_device()())
      : m_p(p), m_n(n) {
    assert(m_p >= 0.0);
    assert(m_p <= 1.0);
  ...
  double sample() override { return gsl_ran_binomial(m_rng, m_p, m_n); }
...
struct Pareto : RV {
  ...
  Pareto(const gsl_rng_type *rngType, double a = 1.0, double b = 1.0,
         unsigned long int seed = std::random_device()())
      : m_a(a), m_b(b) {
    assert(m_a > 0.0);
    assert(m_b > 0.0);
  ...
  Pareto(double a = 1.0, double b = 1.0,
         unsigned long int seed = std::random_device()())
      : m_a(a), m_b(b) {
    assert(m_a > 0.0);
    assert(m_b > 0.0);
  ...
  double sample() override { return gsl_ran_pareto(m_rng, m_a, m_b); }
  double mean() override {
    if (m_a <= 1.0) {
      return std::numeric_limits<double>::infinity();
    } else {
  ...
  double stdv() override {
    if (m_a <= 2.0) {
      ...
      return sqrt((pow(m_b, 2) * m_a) / (pow(m_a - 1.0, 2) * (m_a - 2.0)));
    }
  }
...
struct ChiSquared : RV {
  ...
  ChiSquared(const gsl_rng_type *rngType, double nu = 1.0,
             unsigned long int seed = std::random_device()())
      : m_nu(nu) {
    assert(m_nu > 0.0);
    m_rng = gsl_rng_alloc(rngType);
  ...
  ChiSquared(double nu = 1.0, unsigned long int seed = std::random_device()())
      : m_nu(nu) {
    assert(m_nu > 0.0);
    m_rng = gsl_rng_alloc(gsl_rng_ranlxd2);
    gsl_rng_set(m_rng, seed);
  }
  ~ChiSquared() { gsl_rng_free(m_rng); }
  void set_rng(gsl_rng *newRNG) override {
    gsl_rng_free(m_rng);
    m_rng = newRNG;
  }
  ...
  double sample() override { return gsl_ran_chisq(m_rng, m_nu); }
...
struct TStudent : RV {
  ...
  TStudent(const gsl_rng_type *rngType, double nu = 1.0,
           unsigned long int seed = std::random_device()())
      : m_nu(nu) {
    assert(m_nu > 0.0);
    m_rng = gsl_rng_alloc(rngType);
  ...
  TStudent(double nu = 1.0, unsigned long int seed = std::random_device()())
      : m_nu(nu) {
    assert(m_nu > 0.0);
    m_rng = gsl_rng_alloc(gsl_rng_ranlxd2);
    gsl_rng_set(m_rng, seed);
  }
  ~TStudent() { gsl_rng_free(m_rng); }
  void set_rng(gsl_rng *newRNG) override {
    gsl_rng_free(m_rng);
    m_rng = newRNG;
  }
  ...
  double sample() override { return gsl_ran_tdist(m_rng, m_nu); }
...
struct FDistribution : RV {
  ...
  FDistribution(const gsl_rng_type *rngType, double nu1 = 1.0, double nu2 = 1.0,
                unsigned long int seed = std::random_device()())
      : m_nu1(nu1), m_nu2(nu2) {
    assert(m_nu1 > 0.0);
    assert(m_nu2 > 0.0);
  ...
  FDistribution(double nu1 = 1.0, double nu2 = 1.0,
                unsigned long int seed = std::random_device()())
      : m_nu1(nu1), m_nu2(nu2) {
    assert(m_nu1 > 0.0);
    assert(m_nu2 > 0.0);
  ...
  ~FDistribution() { gsl_rng_free(m_rng); }
  void set_rng(gsl_rng *newRNG) override {
    gsl_rng_free(m_rng);
    m_rng = newRNG;
  }
  ...
  double sample() override { return gsl_ran_fdist(m_rng, m_nu1, m_nu2); }
  double mean() override {
    if (m_nu1 > 2.0) {
...
struct Shuffler {
  ...
  Shuffler(const gsl_rng_type *rngType = gsl_rng_ranlxd2,
           unsigned long int seed = std::random_device()()) {
    m_rng = gsl_rng_alloc(rngType);
    gsl_rng_set(m_rng, seed);
  }
  ~Shuffler() { gsl_rng_free(m_rng); }
  void set_rng(gsl_rng *newRNG) {
    gsl_rng_free(m_rng);
    m_rng = newRNG;
  }
  void set_seed(unsigned long int seed) { gsl_rng_set(m_rng, seed); }

  template <typename Vector> void shuffle(Vector &vec) {
...
struct Dice {
  ...
  Dice(const gsl_rng_type *rngType, int nSides,
       unsigned long int seed = std::random_device()())
      : m_nSides(nSides) {
    m_unifInt = UniformInt(rngType, 1, m_nSides, seed);
  }
  Dice(int nSides) : m_nSides(nSides) { m_unifInt = UniformInt(1, m_nSides); }
  double sample() { return m_unifInt.sample(); }
...
struct SphericalVectors {
  ...
  SphericalVectors(const gsl_rng_type *rngType, size_t nd = 2,
                   unsigned long int seed = std::random_device()())
      : m_nd(nd) {
    assert(m_nd >= 1);
    m_rng = gsl_rng_alloc(rngType);
  ...
  ~SphericalVectors() { gsl_rng_free(m_rng); }
  void set_rng(gsl_rng *newRNG) {
    gsl_rng_free(m_rng);
    m_rng = newRNG;
  }
